...
#
APP_VERSION = "1.1.7"  # bump this string with each GitHub release you publish
...
def fetch_flips(category_map, overrides, bazaar_data):
    ...
    for product_id, info in products.items():
        ...
        # Prices come from the actual order book (buy_summary /
        # sell_summary), NOT quick_status.buyPrice/sellPrice. Hypixel's
        # quick_status prices are volume-weighted averages over the top 2%
        # of the book, so one whale wall parked under the real top order
        # drags them far away from anything actionable (observed: Third
        # Master Star showing "buy at 18m" while the true top buy order
        # sat at 26m all week - no order was ever fillable at 18m, the
        # number was a statistical artifact of a low-ball wall). The
        # literal book tops are the two numbers a flipper actually has to
        # beat: buy_summary[0] = lowest current sell offer (insta-buy
        # price), sell_summary[0] = top current buy order (insta-sell
        # price). Missing/empty summaries fall through as 0 and get
        # dropped by the <= 0 guard below, same as before.
        buy_summary = info.get("buy_summary") or []
        sell_summary = info.get("sell_summary") or []
        buy_price = float(buy_summary[0].get("pricePerUnit", 0)) if buy_summary else 0.0     # top sell offer
        sell_price = float(sell_summary[0].get("pricePerUnit", 0)) if sell_summary else 0.0  # top buy order
...
class EventItemCard(tk.Frame):
    ...
    `live` (optional) is the price-movement-so-far dict built by
    _live_event_item_stats for items of a still-open occurrence; it lets the
    card show how the item has fluctuated during the live event even when
    there's no prior occurrence to score a Buy/Hold/Sell against yet."""
    def __init__(self, parent, item_id, recommendation, live=None):
        super().__init__(parent, bg=BORDER_SUBTLE)
        self.expanded = False
        self.detail = None
        ...
        if recommendation:
            ...
            buy_side = live.get("buy_order")
            if buy_side:
                quick_text = (f"instabuy {live['pct_change']:+.1f}% \u00b7 "
                              f"buy order {buy_side['pct_change']:+.1f}% since event start")
            else:
                quick_text = (f"{live['pct_change']:+.1f}% since event start \u00b7 "
                              f"{live['baseline']:,.1f} \u2192 {live['current']:,.1f}")
            confidence_color = action_color
        else:
    ...
    def _build_detail(self):
        ...
        if self.live is not None:
            lv = self.live
            live_grid = tk.Frame(self.detail, bg=BG_PANEL_RAISED)
            live_grid.pack(fill="x", padx=16, pady=(10, 6))
            live_rows = [
                ("Instabuy At Start → Now",
                 f"{lv['baseline']:,.1f} → {lv['current']:,.1f} ({lv['pct_change']:+.1f}%)"),
                ("Instabuy High / Low This Event",
                 f"{lv['high']:,.1f} / {lv['low']:,.1f}"),
            ]
            buy_side = lv.get("buy_order")
            if buy_side:
                live_rows += [
                    ("Buy Order At Start → Now",
                     f"{buy_side['baseline']:,.1f} → {buy_side['current']:,.1f} "
                     f"({buy_side['pct_change']:+.1f}%)"),
                    ("Buy Order High / Low This Event",
                     f"{buy_side['high']:,.1f} / {buy_side['low']:,.1f}"),
                ]
            live_rows.append(("Samples Recorded This Event", str(lv["n_samples"])))
            for i, (label, text) in enumerate(live_rows):
                r, c = divmod(i, 2)
                cell = tk.Frame(live_grid, bg=BG_PANEL_RAISED)
                cell.grid(row=r, column=c, sticky="w", padx=(0, 28), pady=2)
                tk.Label(cell, text=label + ":", font=FONT_MAIN, bg=BG_PANEL_RAISED,
        ...
        if rec is None:
            ...
            tk.Label(self.detail, text=note,
                     font=FONT_MAIN, bg=BG_PANEL_RAISED, fg=TEXT_DIM,
...
class BazaarFlipperApp(tk.Tk):
    ...
    def _gather_event_sections(self):
        ...
        Items of a still-open occurrence additionally get a "live" stats
        dict (price at event start vs now, high/low, sample count) built
        straight from the price history recorded during the occurrence -
        so a first-ever occurrence with nothing to score against still
        shows how each tracked item has moved since the event began."""
        ...
        for event_type in event_types:
            ...
            is_ongoing = current_instance.end_ts is None
            ...
            for item_id in item_ids:
                try:
                    ...
                    rec = None
                live = (self._live_event_item_stats(pipeline_obj, item_id, current_instance, as_of_ts)
                        if is_ongoing else None)
                if rec is not None or live is not None:
                    item_rows.append({"item_id": item_id, "recommendation": rec, "live": live})
    ...
    def _live_event_item_stats(self, pipeline_obj, item_id, instance, as_of_ts):
        """Price movement for one item during a still-open event occurrence:
        baseline (last sample within 24h before the event started, else the
        first in-event sample), current price, % change, and the high/low
        seen so far.

        Primary source is the app's own price history (self.price_history),
        whose samples carry BOTH sides of the book - raw_buy_target (top
        buy order) and raw_sell_target (top sell offer, i.e. the instabuy
        price) - so both sides' movement can be shown. The engine's DuckDB
        store only keeps the sell-offer side (see db._ingest_sample_map),
        so it's the fallback when the app history has nothing for this
        item, with the buy-order side omitted. The top-level keys are the
        instabuy side; the buy-order side sits under "buy_order" (None
        when unavailable). Returns None (fails open) if nothing has been
        sampled during the event yet, so callers can fall back to their
        no-data note."""
        start_ts = instance.start_ts
        window_lo = start_ts - 24 * 3600

        def _side_stats(points):
            """points: ascending [(ts, value)] covering [window_lo, as_of]."""
            during = [(ts, v) for ts, v in points if ts >= start_ts]
            if not during:
                return None
            pre = [(ts, v) for ts, v in points if ts < start_ts]
            baseline = pre[-1][1] if pre else during[0][1]
            current = during[-1][1]
            values = [v for _ts, v in during]
            pct = ((current - baseline) / baseline * 100.0) if baseline > 0 else 0.0
            return {"baseline": baseline, "current": current, "pct_change": pct,
                    "high": max(values), "low": min(values), "n_samples": len(during)}

        app_samples = [s for s in getattr(self, "price_history", {}).get(item_id, [])
                       if window_lo <= s[0] <= as_of_ts]
        sell = _side_stats([(s[0], s[2]) for s in app_samples if len(s) > 2])
        buy = _side_stats([(s[0], s[1]) for s in app_samples if len(s) > 1])

        if sell is None:
            buy = None
            try:
                points = pipeline_obj.db.load_price_history(
                    item_id, start_ts=window_lo, end_ts=as_of_ts)
            except Exception:
                return None
            sell = _side_stats([(p.ts, p.price) for p in points])
            if sell is None:
                return None
    ...
    def _render_event_engine(self):
        ...
        for section in sections:
            rows = [row for row in section["items"] if not query or query in row["item_id"].lower()]
            is_ongoing = section["current_instance"].end_ts is None
            # Ongoing events always get their section shown (unless a search
            # filtered its items away) - a first-ever occurrence has nothing
            # scoreable yet, but its live tracking still belongs on screen.
            if not rows and not (is_ongoing and not query):
                continue
            rows.sort(key=self._event_row_sort_value, reverse=self.event_sort_reverse)
            self._render_event_section(section, rows)
            sections_rendered += 1
            shown += sum(1 for r in rows if r["recommendation"] is not None)
            live_shown += sum(1 for r in rows if r["recommendation"] is None)

        if not shown and not live_shown and not sections_rendered and not upcoming_shown:
        ...
        if live_shown:
            count_text += f" · {live_shown} live-tracked"
        if upcoming_shown:
    ...
    def _event_row_sort_value(self, row):
        ...
        if self.event_sort_key == "movement":
            ...
            live = row.get("live")
            return live["pct_change"] if live else 0.0
        if rec is None:
            return 0.0
    ...
    def _render_event_section(self, section, rows):
        ...
        if not rows:
            tk.Label(wrap, text="    This event is live - price movement for its tracked items "
                     "appears here as soon as samples are recorded during the event (each bazaar "
                     "refresh while the app is open).",
                     font=FONT_MAIN, bg=BG_DARK, fg=TEXT_DIM, wraplength=1000,
                     justify="left").pack(anchor="w", pady=(2, 4))
        for row in rows:
            EventItemCard(wrap, row["item_id"], row["recommendation"],
                          live=row.get("live")).pack(fill="x", pady=(1, 0))
