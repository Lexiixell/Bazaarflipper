...
def generate_recommendation(conn, item_id: str, upcoming_instance: db.EventInstance,
    ...
    # How much of that same recent window was actually spent with the app
    # open and sampling, vs. closed -- momentum/deviation are read off
    # recent_prices above, so this tells session_coverage_dampener how much
    # to trust that read. Fails open (1.0) if app_sessions has nothing to
    # say about it (see db.session_coverage), same as every other dampener
    # here when its underlying data is unavailable.
    try:
        coverage = db.session_coverage(conn, as_of_ts - RECENT_WINDOW_DAYS * 86400, as_of_ts)
    except Exception:
        coverage = 1.0
...
def generate_recommendation_for_forecast(conn, item_id: str, event_type: str, next_start_ts: int,
                                           min_liquidity_volume: float = DEFAULT_MIN_LIQUIDITY_VOLUME,
                                           as_of_ts: Optional[int] = None,
                                           indicators=None) -> Optional[scoring.Recommendation]:
    """Score a PRE-EVENT Buy/Hold/Sell signal for an event that hasn't started
    yet but is forecast to begin at `next_start_ts` (see bazaarflipper's
    forecast_events / bazaar_bridge's event_forecast sync).

    The trick is that generate_recommendation already indexes everything by
    relative day off the instance's start_ts, so a synthetic instance whose
    start is in the FUTURE simply puts "today" at a negative relative day
    (e.g. as_of ~24h before start -> day -1). That's exactly where the
    historical buy window tends to live (event_study's BASELINE_WINDOW /
    search window run from day -30), so the same scoring path that grades a
    live event now grades "should I pre-position for the one that's coming."

    The synthetic instance is never written to the `events` table -- it exists
    only for this scoring call, so real-occurrence history stays clean and
    there's nothing to reconcile when the event actually starts and gets
    recorded for real by the bridge. Returns None (fails open) exactly when
    generate_recommendation would: no prior recorded occurrences of this event
    type, or no price history for the item yet."""
    synthetic = db.EventInstance(
        event_instance_id="{0}:forecast:{1}".format(event_type, int(next_start_ts)),
        event_type=event_type,
        start_ts=int(next_start_ts),
        end_ts=None,
    )
    return generate_recommendation(conn, item_id, synthetic, min_liquidity_volume,
                                    as_of_ts, indicators)
